...
### 核心功能
- **資產轉移管理** - 支援批次資產轉移申請與審核流程，可分別變更保管人、使用人或存放地點
- **彈性轉移選項** - 支援三種轉移類型：地點變更、保管人變更、使用人變更（可單獨或組合使用）
...
- 📊 **動態數據流** - 自動從資產清單提取保管人、使用人、地點等參考資料，無需維護額外對照表
- ✉️ **智慧通知機制** - 根據轉移類型（保管人/使用人/地點）自動發送 Email 通知相關人員
- 🎯 **彈性轉移機制** - 支援保管人、使用人、地點的獨立或組合變更，自動判斷是否需要審核
...
- 後端：`processBatchTransferApplication()` (code.js:599-878)
...
- 後端：`processBatchLending()` (code.js:1304-1435)
...
- 後端：`processBatchScrapping()` (code.js:1513-1926)

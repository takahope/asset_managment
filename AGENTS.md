# Repository Guidelines

## Project Structure & Module Organization
This is a Google Apps Script (GAS) web app stored at the repo root.

- `code.js`: Primary GAS backend logic (asset data, transfers, lending, scrapping, inventory).
- `dashboard_code.js`: Dashboard-specific helpers and data shaping.
- `deploy.js`: One-time sheet setup helpers (run `deployAllSheets()` from GAS).
- `env.js`: Environment IDs for Sheets/Docs/Folders used by the app.
- `*.html`: UI templates (e.g., `Inedex.html`, `dashboard.html`, `selectbyqr.html`).
- `appsscript.json`: GAS manifest (runtime, web app settings).
- `.clasp.json`: Local clasp configuration for syncing with GAS.

## Build, Test, and Development Commands
Use clasp to sync local files with the Apps Script project:

- `clasp login`: authenticate with Google (one-time per machine).
- `clasp pull`: fetch the current script from GAS.
- `clasp push`: upload local changes to GAS.
- `clasp deploy`: create a new deployment/version.
- `clasp open`: open the script in the Apps Script editor.

For sheet initialization, run `deployAllSheets()` in `deploy.js` from the Apps Script editor.

## Coding Style & Naming Conventions
- Indentation: 2 spaces in JS/HTML.
- Naming: `camelCase` for functions/variables, `SCREAMING_SNAKE_CASE` for constants.
- Keep top-level constants grouped (sheet names, column indices) as in `code.js`.
- Comments are concise and often in Traditional Chinese; keep tone consistent.

## Testing Guidelines
No automated test framework is configured. Validate changes manually:
- Test web app flows in the GAS UI (transfer, lending, scrap, inventory).
- Verify Sheet writes and timestamps in the target spreadsheet.

## Commit & Pull Request Guidelines
This workspace does not include Git history. If you create commits, use short, imperative messages and include scope when helpful (e.g., `inventory: fix export dates`).

PRs should include:
- A clear description of the change and impacted flows.
- Screenshots for UI changes (`dashboard.html`, `selectbyqr.html`, etc.).
- Any related Sheet schema changes or required environment updates.

## Security & Configuration Tips
- Update IDs in `env.js` for your environment before deploying.
- Web app access is domain-restricted in `appsscript.json`; confirm this matches deployment needs.

- 2026-09-18: Separated scrap reason (Column K) and application method (Column Q: "申請方式") in ScrapLog and master sheet REMARKS. Column K now exclusively records pure user reason codes (A, B, or C with details), while Column Q records proxy application details ('本人申請' or '同組代辦 (代辦人: XXX)'). ScrapPrintModal in alpine_modals_print.html now displays the "申請方式" column with 3-state sorting and keyword search integration. Added unit test testScrapReasonAndApplyMethodSeparation_() in code.js.
- 2026-09-18: Implemented 8-column sort, keyword search, and batch scrap cancellation ("取消選取的報廢") for ScrapPrintModal ("待報廢資產") in alpine_modals_print.html and processBatchCancelScrap in code.js. Restores master status to '在庫', clears remarks/lastModified, marks ScrapLog as '已取消', and refreshes pending list. Added unit test function testBatchCancelScrapLogic_().
- 2026-09-18: Implemented 12-column sort and keyword instant search for TransferPrintModal ("待列印清單") in alpine_modals_print.html. Table header columns support three-state sorting (asc -> desc -> reset), and top toolbar features a compact layout with category radios, keyword search, and item counter. Linked handleSelectAll with filteredAssets. Added scoped CSS (.transfer-sortable-th, .transfer-search-*) preventing frozen Tailwind omissions.
- 2026-09-17: Implemented transfer rollback feature ("退回轉移") in TransferPrintModal (`alpine_modals_print.html`) and backend handler `processBatchTransferRollback` in `code.js`. Allows operators to rollback accepted/location-transferred assets prior to document printing, restoring original keeper (and email), original user (and email), and original location, re-marking IS_COMPUTER, resetting isUploaded to 'V', and updating application log status to "已退回". Added unit test function `testTransferRollbackLogic_()`.
- 2026-09-15: Resolved severe loading latency regression (userStateMs 129.6s -> 82.9s -> <10ms). Identified that unmemoized `getHrGroupNameMap_` in `isms-connect-asset/code.js` or unsynced `hr_directory.js` was still causing 4,630 remote PropertiesService RPCs in `.map()`. Implemented full defense-in-depth: added `CANONICAL_GROUP_NAME_MAP_MEMO_` and `CANONICAL_GROUP_CACHE_` in `code.js`, added memoization to `isms-connect-asset/code.js`, and short-circuited permission calculation in `getUserStateData` so Admin and Owner assets completely bypass `isAssetInUserGroupScope_`. Reduced 4,630 assets execution time to 7ms.
- 2026-09-15: Added "Info-Out" transfer mode (`isInfoOutTransfer`) to single and batch transfer modals (`alpine_modals_single.html`, `alpine_modals_batch.html`) and backend handler in `code.js`. Operators select a single recipient to simultaneously update both keeper and user with optional location change, logged as "資訊組轉出".
- 2026-09-15: Implemented dual-track union permission model for AE column (DEFAULT_GROUP) assets in code.js. Defined helpers isAssetInUserGroupScope_ and canonicalizeGroupName_ to allow group members (asset ownership track) and keeper peers (custody track) to view, transfer, lend, return, and scrap designated project assets when group collaboration is enabled. Preserved DEFAULT_GROUP unchanged on transfer. Added unit test function testDefaultGroupAssetCollaborationMatrix_().
- 2026-09-15: Optimized single and batch transfer modals (`quickTransferModal` in `alpine_modals_single.html`, `batchTransferModal` in `alpine_modals_batch.html`) by introducing `availableKeepers` and `availableUsers` getters to automatically exclude existing keepers and users from dropdown options (Batch transfer adopts Strategy A: union set exclusion). Added frontend submission validation guard clauses to prevent submitting no-op keeper/user assignments.
- 2026-09-08: Fixed pending scrap print modal (`ScrapPrintModal`) and drawer card count (`cardCounts.scrapPending`) not showing group members' pending scrap items. Corrected undeclared variable `groupProxyEnabled` in `getAllScrappableItems` to `groupProxyScrapEnabled = !useAdminScope && isGroupProxyScrapEnabled()`, added `groupProxyScrapEnabled` check in `getTransferData` group email preparation, and updated `alpine_store.html` `cardCounts.scrapPending` to read `groupProxyScrapEnabled`.
- 2026-09-08: Fixed DocumentApp '無法插入空白文字元素' (Cannot insert empty text element) runtime error during Doc generation for non-consumables in `createTransferDoc`, `createScrapDoc`, and `createLendingDoc`. Defined `safeDocText_` helper to fallback empty strings/null/undefined to `' '` (satisfying Google Docs length >= 1 constraint). Fixed `createTransferDoc` using `insertTableRow(insertPosition)` instead of `appendTableRow()`. Updated `gas-google-docs-table-generation` skill Problem 11.
- 2026-09-07: Refactored group collaboration settings into 3 independent tiers: View (`GROUP_VIEW_ENABLED`), Proxy Transfer/Lend (`GROUP_PROXY_TRANSFER_LEND_ENABLED`), and Proxy Scrap (`GROUP_PROXY_SCRAP_ENABLED`) with hierarchy interlock in `code.js` and `alpine_model_setting.html`. Extended DTO with `canTransfer`, `canLend`, `canScrap`, and updated `getUserStateData`, `getAssetsForCurrentUser`, `processBatchLending`, `processBatchReturn`, `processBatchScrapping`, `cancelTransferOrScrap`. Added original keeper notification email and ScrapLog proxy prefix for proxy scrapping. Updated Action Panel and quick action buttons in `alpine_views.html`, `alpine_inventory_modals.html`, `alpine_inventory_script.html`, `alpine_store.html`. Added test function `testGroupCollaborationMatrix_()`.
- 2026-09-07: Added simulated progress bar and dynamic status text to batch processing cart footer (`batch_processing_list.html`, `alpine_cart.html`, `alpine_store.html`). Inserted `#BatchList_progressArea` between "Clear" button and item count with scoped CSS (`.batch-list-progress-area`, `.batch-list-progress-fill`). When "送出清單" (submit list) is clicked in advanced batch mode, progress bar advances smoothly to ~80% over ~5 seconds with rotating messages, disables action buttons to prevent double-clicks, animates swiftly to 100% on completion, and auto-fades after 1.5 seconds.
- 2026-08-24: Fixed vertical scrolling and layout clipping in print modals (`TransferPrintModal`, `ScrapPrintModal`, `LendingPrintModal`) in `alpine_modals_print.html`. Resolved root cause where `<template x-if="activeTab === 'pending'">` inserted an unconstrained block `<div>` breaking flex child calculations; added scoped CSS classes `.print-modal-tab-body` and `.print-modal-scroll-area` with `min-height: 0` to enable proper flex shrinking and vertical scrolling within `max-h-[90vh]`, added `shrink-0` to headers/tabbars/footers, and added `z-10 shadow-sm` to sticky table headers.
- 2026-08-21: Implemented bi-directional smart linkage for station transfer modal dropdowns ("Resident Custodian" and "Station Location"). Updated `code.js` and `hr_directory.js` to parse manager names from column H of HR "組織架構樹" (for permanent & outsourced stations) and manager emails from column C of "行動駐站" sheet (for portable stations), mapping them to active employees. Expanded `getTransferData` to return `stationToCustodians` and `custodianToStations`. Updated `quickTransferModal` in `alpine_modals_single.html` and `batchTransferModal` in `alpine_modals_batch.html` with reactive getters and change handlers to support 1:1 auto-selection and 1:N option filtering.
- 2026-08-14: Implemented batch actions (Transfer, Scrap, Lending) for "Needs Confirmation" (location error) modal. Added checkboxes, top action bar to `alpine_modals_location_error.html`. Updated `alpine_store.html` to manage selected IDs and remove processed items inline. In `code.js`, implemented `silentResolveLocationError_` and integrated it into `processBatchTransferApplication`, `processBatchLending`, and `processBatchScrapping` to sync backend resolution state automatically.
- 2026-08-12: Replaced `getAdminEmails()` with `getDataUpdateEmails()` in `code.js` for location change, transfer approval, and scrap application notifications, separating admin roles from data update contact notifications.
- 2026-08-11: Added HR Group Name Mapping management Tab to `connect.html` settings modal, with backend endpoints `getHrGroupSettings()` and `saveHrGroupNameMap()` in `code.js`. Enables administrators to maintain `HR_GROUP_NAME_MAP` (HR org names → system display names) via UI, fixing ISO scope scan station name mismatch.
- 2026-07-31: Fixed mobile bottom action bar (`actionBar`) layout in `connect.html`: restructured elements to make search and remarks inputs full-width by applying scoped CSS classes (`max-width: 100% !important` to override base `300px` limit) and removing inline styles, and grouped edit/delete buttons inline with the business process dropdown on mobile.
- 2026-07-31: Fixed mobile scan panel (`embeddedActionPanel`) layout disorder in `connect.html`: global `.search-input { order: -1 }` was pushing search/remarks inputs above the status card; scoped rule to `.card .flex.flex-wrap .search-input`, moved status card + meta outside panel body to between scannerStatus and panel header, swapped remarks/business-process order in bottom action bar.
- 2026-07-29: Added B219 HW Label Filter to `connect.html` alongside the station checkbox, enabling filtering by confidentiality level mapping for B219 hardware assets.
- 2026-07-29: Added a dedicated "Hardware Label" filter dropdown to `index.html` for identifying B219 HW assets requiring specific confidentiality labels (Blue=Level 4, Green=Level 3).
- 2026-07-29: Fixed issue in `iso_scope.js` where virtual records (`assetId: '無對應資產/消耗品'`) representing empty ISMS assets were incorrectly treated as unmapped physical assets and queued for auto-creation.
- 2026-07-29: Fixed missing "entering scope" (進入範圍) asset list display in isoScanModal of `connect.html`.
- 2026-07-23: Added "Tag Error" (`tag`) type alongside "Location Error" and "Keeper Error", with dedicated sky-colored action button, distinct notification email copy, modal sky badge, and updated Sheet export mapping.
- 2026-07-23: Added "Keeper Error" type alongside "Location Error", with dedicated action buttons, distinct notification email copy, and expanded CSV export header schema.
- 2026-07-20: Replaced single-select Font Awesome chevrons in `alpine_views.html` with CSS-drawn chevrons to avoid missing icon rendering.
- 2026-07-20: Fixed `alpine_views.html` native select icon positioning by replacing missing frozen Tailwind `right-3` reliance with scoped CSS.
- 2026-07-20: Added scoped responsive CSS for `alpine_views.html` filter toolbar to avoid missing frozen Tailwind `lg:*` utilities breaking desktop layout.
- 2026-07-20: Fixed duplicate native/custom select arrows in `alpine_views.html` filter selects with scoped appearance reset CSS.
- 2026-07-19: Fixed Alpine.js `x-model` and dynamic `<option>` race condition in `alpine_model_setting.html` by using `$nextTick` to set select models.

## ★ Insight ─────────────────────────────────────
- 沉澱時發現 gas-fullstack:476 早已寫過「凍結 Tailwind → 寫具名 scoped CSS class」的通則——這正解釋了為何當初該 skill 沒被觸發：這次是新功能作者沒讀既有規範。所以我補的是更難察覺的新症狀（z-index 任意值 → 遮蓋），提高未來被 grep/掃到的機率，而非複述通則。
- 三層沉澱各司其職：PLAYBOOK §4=跨專案速記、skill=詳解單一來源、專案 CLAUDE.md=在地事件溯源。同一課三處互相指引，日後任一入口都找得到全貌。

**⚠️ 前端修改鐵則** 
修改前端時，一定要檢查掃描現有已編譯的凍結 Tailwind。如果沒有可用的 class，就**必須**使用「具名 scoped CSS class」，以免造成預期外的樣式錯誤。
